...
- **Visible AI markers** — an optional `[AI]` title prefix becomes a compact badge while task names stay clean in the tracker
...
- `[AI] Draft Jorge click-fraud email` — the task can still include a human approval or action checkpoint.
- `Create ShinePod video V1` — no prefix means human-led.

The marker stays in the actual Notion title, so it remains visible in Notion Home's My Tasks view. The tracker removes the recognized prefix from its displayed task name and shows one violet `AI` badge beside it. Normal task lists, global search results, and recent tasks all use the same parser. The retired `[AI review]` prefix temporarily renders as the same `AI` badge for backwards compatibility; new and replanned tasks should use `[AI]`. Malformed or embedded bracket text is left unchanged.
